...
contains
 ...
 elemental  function deq(x,y) result (test)
    real(kind=dp), intent(in) :: x,y
    logical :: test
    test = abs(x-y) <= max(abs(x),abs(y))*eps
  end function deq

 elemental  function dgt(x,y) result (test)
    real(kind=dp), intent(in) :: x,y
    logical :: test
    test = (x-y) > max(abs(x),abs(y))*eps
  end function dgt

  elemental  function dne(x,y) result (test)
    real(kind=dp), intent(in) :: x,y
    logical :: test
    test = .not. deq(x,y)
  end function dne

  elemental  function dle(x,y) result (test)
    real(kind=dp), intent(in) :: x,y
    logical :: test
    test = .not. dgt(x,y)
  end function dle

  elemental  function dlt(x,y) result (test)
    real(kind=dp), intent(in) :: x,y
    logical :: test
    test = dle(x,y) .and. dne(x,y)
  end function dlt

  elemental  function dge(x,y) result (test)
    real(kind=dp), intent(in) :: x,y
    logical :: test
...
! compare with 0  
 elemental  logical function dGtZero(x)
    real(kind=dp), intent(in) :: x
    dGtZero =  dgt(x, 0._dp)
  end function dGtZero
 
 elemental  logical function dLtZero(x)
    real(kind=dp), intent(in) :: x
    dLtZero =  dlt(x, 0._dp)
...
write(*,*) ' eps=',eps
 ...
 elemental  logical function dNeqZero(x)
    real(kind=dp), intent(in) :: x
